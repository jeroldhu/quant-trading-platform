...
- `uv run mypy src/quant_trading`：43 个源文件 strict 检查通过。
- `uv run quant config validate`：pipeline、交易日历、资产池、回测和报告配置通过。
- `uv run quant research strategies validate`：3 个显式注册策略通过。
- 离线 ETF Raw→Gold、版本父链、61 日门禁、历史回测和报告 bundle：通过。
- 离线主题快照→股票 Gold→主题内选股：180 行 qfq、2 个目标；早于首份快照的
  查询明确阻断，point-in-time guard 通过。
- 离线股票管道：180 行 Gold，`STOCK_BACKTEST_READY=READY`。
- 实源单标的采集已验证成功与必需源失败阻断两条路径；全市场生产验收不属于代码
  TODO 的完成条件，按 `docs/operations.md#9-发布验收清单` 留存运行证据。
